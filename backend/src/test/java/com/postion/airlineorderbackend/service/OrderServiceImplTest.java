...
//    }
    ...
    // assertEquals("Order is already cancelled。orderID: 100",
    ...
    // RuntimeException exception = assertThrows(RuntimeException.class,
    // () -> orderService.cancelOrder(100L));
    // assertEquals("Completed orders cannot be cancelled。orderID: 100",
    // exception.getMessage());
    //
    // verify(orderRepository, times(1)).findById(100L);
    // verify(orderRepository, never()).save(any());
    ...
    // when(orderRepository.save(testOrder)).thenReturn(testOrder);
    //
    // OrderDto result = orderService.updateStatus(100L,
