...
DROP TABLE IF EXISTS `orders_xyl`;
DROP TABLE IF EXISTS `app_users_xyl`;

CREATE TABLE `app_users_xyl` (
...
CREATE TABLE `orders_xyl` (
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_orders_user_id_xyl` FOREIGN KEY (`user_id`) REFERENCES `app_users_xyl` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
INSERT INTO `app_users_xyl` (`id`, `username`, `password`, `role`) VALUES
...
-- 插入覆盖所有场景的订单数据
INSERT INTO `orders_xyl` (`order_number`, `status`, `amount`, `creation_date`, `user_id`) VALUES
...
SELECT COUNT(*) AS '用户总数' FROM `app_users_xyl`;
